...
CanvasDisplay.prototype.updateViewport = function (state) {
  ...
  } else if (center.y > view.typ + view.height - margin) {
...
otherSprites.src = base + "img/sprites.png";
...
playerSprites.src = base + "img/player.png";
...
async function runGame(plans, Display) {
  ...
  for (let level = 0; level < plans.length; ) {
    ...
    if (status == "won") level++;
    if (status === "lost") {
      console.log(`${--lives} ${lives === 1 ? "life" : "lives"} left`);
      if (lives <= 0) {
        break;
      }
    }
  }
  console.log(lives ? "You've won!" : "Game over");
}
